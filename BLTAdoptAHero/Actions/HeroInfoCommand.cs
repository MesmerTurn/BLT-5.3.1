...
namespace BLTAdoptAHero
{
    ...
    internal class HeroInfoCommand : ICommandHandler
    {
        private class Settings : IDocumentable
        {
            ...
            [LocDisplayName("Show Prestige"),
             LocDescription("Shows prestige level, kills toward next prestige, and cumulative bonuses."),
             PropertyOrder(13), UsedImplicitly]
            public bool ShowPrestige { get; set; } = true;
        ...
        void ICommandHandler.Execute(ReplyContext context, object config)
        {
            ...
            else
            {
                ...
                if (settings.ShowPrestige)
                {
                    var prestigeCfg = BLTAdoptAHeroModule.CommonConfig.PrestigeConfig;
                    int pLevel = BLTAdoptAHeroCampaignBehavior.Current.GetPrestigeLevel(adoptedHero);
                    int pKills = BLTAdoptAHeroCampaignBehavior.Current.GetPrestigeKillCount(adoptedHero);
                    string title = prestigeCfg.GetChatTitle(pLevel);
                    int goldBonus = prestigeCfg.GetCumulativeGoldBonusPercent(pLevel);
                    float xpMult = prestigeCfg.GetCumulativeXPMultiplier(pLevel);

                    string prestigeStr;
                    if (pLevel == 0)
                    {
                        prestigeStr = $"[PRESTIGE] None | Kills: {pKills}/{prestigeCfg.RequireKills}";
                    }
                    else if (pLevel >= prestigeCfg.MaxPrestigeLevel)
                    {
                        prestigeStr = $"[PRESTIGE] {title} MAX | +{goldBonus}% gold/kill | {xpMult:F1}x XP";
                    }
                    else
                    {
                        int killsNeeded = prestigeCfg.RequireKills - pKills;
                        prestigeStr = $"[PRESTIGE] {title} | Kills to next: {pKills}/{prestigeCfg.RequireKills} | +{goldBonus}% gold/kill | {xpMult:F1}x XP";
                    }
                    infoStrings.Add(prestigeStr);
                }
